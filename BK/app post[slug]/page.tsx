...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const publishedDate = (post as any).date 
    ...
    <>
      ...
      <main className="min-h-screen bg-slate-950 text-slate-100 py-10 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          {/* BREADCRUMB - ĐÃ ĐỒNG BỘ ĐƯỜNG DẪN STATIC HUB THEO CATEGORY CHUẨN SEO */}
          <nav className="flex items-center space-x-2 text-xs font-medium text-slate-400 mb-8 overflow-x-auto whitespace-nowrap select-none">
            <Link href="/" className="hover:text-emerald-400 transition-colors">Home</Link>
            <span className="text-slate-600">/</span>
            <Link 
              href={`/categories/${primaryCategory}`} 
              className="hover:text-emerald-400 transition-colors capitalize font-semibold"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
            ...
            <article className="lg:col-span-8 bg-[#0B1121] p-6 sm:p-10 rounded-2xl border border-slate-800 shadow-xl relative overflow-hidden h-fit">
              <div className="absolute top-0 left-0 w-full h-48 bg-linear-to-b from-emerald-900/10 to-transparent pointer-events-none"></div>

              <div className="relative z-10">
                {/* METATAGS - ĐỒNG BỘ ĐỒNG NHẤT SANG SẮC XANH EMERALD */}
                <div className="flex flex-wrap gap-2 items-center text-[10px] font-bold uppercase tracking-wider mb-5">
                  {categories.map((cat) => (
                    <span key={cat} className="px-2.5 py-1 rounded border text-emerald-400 bg-emerald-500/10 border-emerald-500/20 whitespace-nowrap">
                ...
                {/* Title */}
                <h1 className="text-xl sm:text-3xl font-extrabold tracking-tight text-white mb-6 leading-tight">
                  {post.title}
                </h1>
                  ...
                  <div className="bg-slate-900/40 p-4 rounded-xl border border-slate-800/60 space-y-2.5">
                      ...
                      <div className="flex flex-wrap gap-1.5 justify-end">
                        {roles.map((role) => (
                          <Link
                            key={role}
                            href={`/roles/${role}`}
                            className="text-emerald-400 font-semibold text-[1px] border border-emerald-500/20 bg-emerald-500/5 hover:bg-emerald-500/10 hover:border-emerald-400 px-2 py-0.5 rounded whitespace-nowrap transition-all duration-150 cursor-pointer inline-block"
                          >
                            {role.replace(/_/g, " ")}
                          </Link>
                        ))}
                    ...
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <Link
                        ...
                        className="text-emerald-400 hover:text-emerald-300 border border-emerald-500/20 hover:border-emerald-400 font-bold px-2 py-0.5 bg-emerald-500/10 rounded transition-all duration-150 cursor-pointer whitespace-nowrap inline-block"
                      >
                ...
                {/* Content body */}
                <div className="prose prose-slate prose-invert max-w-none prose-headings:text-slate-100 prose-a:text-emerald-400 hover:prose-a:text-emerald-300">
                  <ReactMarkdown
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {roles.map((role) => (
                          <Link
                            key={role}
                            href={`/roles/${role}`}
                            className="text-emerald-400 font-semibold text-[11px] border border-emerald-500/20 bg-emerald-500/5 hover:bg-emerald-500/10 hover:border-emerald-400 px-2 py-0.5 rounded whitespace-nowrap transition-all duration-150 cursor-pointer inline-block"
                          >
                            {role.replace(/_/g, " ")}
                          </Link>
                        ))}
