...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!post) {
    return {
      ...
      description: "The requested QA automation tutorial or interview guide does not exist.",
    };
  }
  ...
  const cleanDescription = `Master the QA Interview: ${post.title}. Blueprint for ${displayRoles || "QA"} positions.`;

  return {
    title: cleanTitle,
    description: cleanDescription,
    keywords: post.tags || ["qa-interview", "software-testing", "qa-lead", ...categoriesArray.map(c => c.replace(/_/g, " "))],
    openGraph: {
      title: cleanTitle,
      description: cleanDescription,
      type: "article",
...
export default async function PostPage({ params }: Props) {
  ...
  // Gọi hàm lấy bài liên quan từ D1 (đã update trong lib/posts.ts)
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <nav className="flex items-center space-x-2 text-xs font-medium text-slate-400 mb-8">
          ...
          <Link href={`/categories/${primaryCategory}`} className="hover:text-emerald-400 capitalize">{primaryCategory.replace(/_/g, " ")}</Link>
          <span className="text-slate-600">/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          <article className="lg:col-span-8 bg-[#0B1121] p-6 sm:p-10 rounded-2xl border border-slate-800">
            {/* 🚀 THÊM LẠI: Metadata Tags (Category, Difficulty, Tools) */}
            ...
            {/* 🚀 THÊM LẠI: Target Roles (Tags nhỏ phía dưới tiêu đề) */}
          ...
          {/* Sidebar và RelatedPosts giữ nguyên */}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2.5">
                      ...
                      <span>•</span>
                      <span>{rp.tool_stack && rp.tool_stack !== 'None' ? rp.tool_stack : displayRpCategory}</span>
                    </div>
